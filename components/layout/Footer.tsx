...
import { Phone } from 'lucide-react'; // Sirf Phone icon yahan se aayega

// --- Custom Social Media SVG Icons ---
const Facebook = ({ size = 24, fill = "none" }) => (
...
const Twitter = ({ size = 24, fill = "none" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill={fill} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/>
  </svg>
);

const Linkedin = ({ size = 24, fill = "none" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill={fill} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const Instagram = ({ size = 24, fill = "none" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill={fill} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
// Helper Component for the Interactive Letters with Tooltips at the bottom
const InteractiveLetter = ({ 
  ...
  return (
    <span className="relative inline-block text-[#f5a623] cursor-default">
      {char}
      <div 
        className={`absolute left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-white px-2 py-1 rounded-sm shadow-lg pointer-events-none whitespace-nowrap z-10 ${
      ...
      >
        ...
        <div className={`absolute left-1/2 -translate-x-1/2 w-2 h-2 bg-white rotate-45 ${
...
export default function Footer() {
  return (
    <footer className="bg-[#0b1727] pt-16 font-sans border-t border-slate-800">
      
      {/* 1. LOCATIONS SECTION */}
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 mb-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center divide-y md:divide-y-0 md:divide-x divide-slate-800/50">
        {locations.map((loc, idx) => (
  <div key={idx} className={`flex flex-col items-center ${idx !== 0 ? 'pt-8 md:pt-0' : ''}`}>
    
    {/* Skyline - height badhayi, white color */}
<div className="h-28 mb-4 w-full flex justify-center items-end">
  <loc.SkylineComponent />

</div>

    {/* Line separator */}
    <div className="w-32 h-px bg-slate-700 mb-6" />
    ...
    <button className="flex items-center gap-3 bg-[#1a2f4e]/40 
                       hover:bg-[#1a2f4e] border border-slate-700/50 
                       text-white px-6 py-2.5 rounded-full 
                       transition-colors duration-300">
      <Phone size={16} className="text-slate-300" />
      <span className="font-medium text-sm">{loc.phone}</span>
    </button>
  </div>
))}
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          ...
          {/* Column 1: Brand & Certificate */}
          <div className="flex flex-col items-start">
            <img src="/images/footer/footer_logo.png" alt="Kotibox" className="h-12  mb-6 object-contain" />
            ...
            <img src="/images/footer/si.jpg" alt="Certificate of Recognition" className="w-48 border-4 border-white/10 rounded-sm" />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-3 mb-10">
              {[Facebook, Twitter, Linkedin, Instagram].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full bg-[#1a2f4e]/60 flex items-center justify-center text-slate-300 hover:bg-[#f5a623] hover:text-white transition-all">
                  {/* Props successfully passed down to custom SVGs */}
                  <Icon size={18} fill={i !== 3 ? "currentColor" : "none"} />
                </a>
              ))}
            </div>

            <h4 className="text-white font-bold text-lg mb-6">Our Ventures</h4>
            <div className="flex gap-4">
              <div className="bg-white p-2 rounded-sm w-32 h-16 flex items-center justify-center">
   ...
   <div className="w-full mx-auto px-6 md:px-12 lg:px-20 pt-8 pb-12 overflow-x-auto whitespace-nowrap text-center [&::-webkit-scrollbar]:hidden">
  <h1 className="w-max mx-auto text-3xl md:text-5xl lg:text-[4rem] xl:text-6xl font-extrabold text-white tracking-tight leading-loose whitespace-nowrap">
